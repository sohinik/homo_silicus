...
class Conjoint(Surveys):
    def __init__(self, *args, num_choices=2, len_choices = 0, **kwds):
    ...
    def build_conjoints(self):
        ...
        all_choice_list_combinations = list(combinations(all_choice_list_keys, self._len_choices))
        all_choice_combinations = []
        for i in all_choice_list_combinations:
            curr_choice_combinations = list(product(*[list(range(len(self._choices_map[j]))) for j in i]))
            for c_i in curr_choice_combinations:
                all_choice_combinations.append(tuple([(i[c_j], c_i[c_j]) for c_j in range(len(c_i))]))
        
        all_choice_combinations_mapped = {}
        for k in all_choice_combinations:
            all_choice_combinations_mapped[k] = [self._choices_map[c_k[0]][c_k[1]] for c_k in k]
        
        final_choices = {}
        final_choices_combinations = list(combinations(all_choice_combinations_mapped.keys(), self._num_choices))
        for l in final_choices_combinations:
            final_choices[tuple(l)] = [self.format_choices(all_choice_combinations_mapped[c_l]) for c_l in l]
