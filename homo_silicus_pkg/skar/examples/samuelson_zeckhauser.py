...
def generate_view():
    ...
    options = [
        ...
        f"{option1} safety is slightly more important than {option2} safety",
...
def run_experiment_samuelson_zeckhauser():
    ...
    for p in prompts:
        print(p)
        result = [i["choice_text"] for i in model.run_prompt(p, temperature=0)]
        print(result)
        print("-----------------------------------------------")
        if len(finals[-1]) >= len(scenarios):
            finals.append([])
        finals[-1].extend(result)
